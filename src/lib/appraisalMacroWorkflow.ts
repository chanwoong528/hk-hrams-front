...
export function normalizeMacroWorkflowPhase(
  ...
  return Math.min(6, Math.max(1, Math.floor(Number(value))));
}

/**
 * 성과 종합 자가(AppraisalBy · performance · 본인)
 * 중간=매크로 3, 기말=매크로 5 (신규 2단계 삽입으로 +1 shift)
 */
...
/** 팀장이 팀원 목표(GoalAssessmentBy)를 작성·수정할 수 있는 단계 — 리더 중간/기말(4·6) */
...
/** 역량 문항 자가 — 성과 자가와 동일 단계(3=중간, 5=기말) */
...
/** 역량 문항 리더 — 성과 리더와 동일 단계(4=중간, 6=기말) */
...
/** 팀장 성과 종합(AppraisalBy performance) — 중간=4, 기말=6 */
