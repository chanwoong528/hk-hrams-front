...
const GoalAssessmentItem = ({
  ...
  const renderEditRow = (key?: string) => (
    <TableRow key={key} className='bg-blue-50/30 ring-1 ring-inset ring-blue-100'>
            ...
            <TableBody>
              {goal.goalAssessmentBy?.map((assessment) => {
                ...
                return (
                  <>
                  <TableRow 
                    key={assessment.goalAssessId} 
                    className='bg-white cursor-pointer hover:bg-gray-50 transition-colors'
                    onClick={(e) => toggleExpansion(assessment.goalAssessId, e)}
                  >
                    <TableCell className='font-medium text-gray-900'>
                      {isMe ? (
                        <div className="flex items-center gap-2">
                           <span className="font-semibold">{assessment.gradedByUser?.koreanName || "본인"}</span>
                           <Badge variant='secondary' className='bg-blue-100 text-blue-700 text-xs px-1.5 py-0 rounded-sm'>Me</Badge>
                        </div>
                      ) : (
                        <span className="font-semibold text-gray-700">{assessment.gradedByUser?.koreanName || "평가자"}</span>
                      )}
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                  ...
                  {isExpanded && assessment.comment && (
                    <TableRow className="bg-gray-50 border-t border-gray-100">
                      <TableCell colSpan={3} className="p-4 text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">
                         {assessment.comment}
                      </TableCell>
                    </TableRow>
                  )}
