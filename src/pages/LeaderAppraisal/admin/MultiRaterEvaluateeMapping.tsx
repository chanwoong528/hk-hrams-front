...
/** X: extra 풀 제거 + 선택 리더와의 수동 연결만 삭제(다른 리더·자동 점선 유지). */
const EXTENDED_POOL_REMOVE_HINT =
  "\ud655\uc7a5 \ud6c4\ubcf4\uc5d0\uc11c \uc81c\uac70\ud569\ub2c8\ub2e4. \uc67c\ucabd\uc5d0\uc11c \ub9ac\ub354\ub97c \uc120\ud0dd\ud55c \uacbd\uc6b0, \uadf8 \ub9ac\ub354\uc640 \uc774 \uc778\uc6d0\uc758 \uc218\ub3d9 \uc5f0\uacb0(\uc2e4\uc120)\ub9cc \uc0ad\uc81c\ub418\uace0 \ub2e4\ub978 \ub9ac\ub354\uc758 \uc218\ub3d9 \uc5f0\uacb0\uc740 \uc720\uc9c0\ub429\ub2c8\ub2e4. \uc790\ub3d9(\uc810\uc120) \uc5f0\uacb0\uc740 \uc720\uc9c0\ub429\ub2c8\ub2e4.";
...
function readDepartmentNamesFromUnknown(raw: unknown): string[] {
  ...
  for (const item of raw) {
    ...
    if (item && typeof item === "object" && "departmentName" in item) {
      const t = readNonEmptyString(
        (item as { departmentName?: unknown }).departmentName,
      );
      if (t) {
        out.push(t);
      }
...
function unwrapGlobalMappingBoardPayload(
  res: unknown,
): LeaderReviewGlobalMappingBoard | null {
...
function readPoolSourceRawFromEvaluateeRecord(
  o: Record<string, unknown>,
): unknown {
  if (
    o.isExtraPool === true ||
    o.fromExtraPool === true ||
    o.inExtraPool === true
  ) {
    return true;
  }
...
function normalizeEvaluateeUserFromApi(raw: unknown): EvaluateeUser | null {
  ...
  const poolSource = normalizeEvaluateePoolSource(
    readPoolSourceRawFromEvaluateeRecord(o),
  );
...
/** API/ORM이 camelCase·snake_case·0/1 등으로 줄 때 매핑 행을 보드 타입으로 맞춤 */
function normalizeMappingsFromApiForBoard(
  mappings: unknown,
): LeaderReviewGlobalMappingBoard["mappings"] {
  if (!Array.isArray(mappings)) {
    return [];
  }
  const out: LeaderReviewGlobalMappingBoard["mappings"] = [];
  for (const raw of mappings) {
    ...
    const o = raw as Record<string, unknown>;
    const leaderRaw =
      o.leaderUserId ?? o.leader_user_id ?? o.LeaderUserId;
    const evalRaw =
      o.evaluateeUserId ?? o.evaluatee_user_id ?? o.EvaluateeUserId;
    const leaderUserId =
      typeof leaderRaw === "string"
        ? leaderRaw.trim()
        : leaderRaw != null && String(leaderRaw).trim()
          ? String(leaderRaw).trim()
          : "";
    const evaluateeUserId =
      typeof evalRaw === "string"
        ? evalRaw.trim()
        : evalRaw != null && String(evalRaw).trim()
          ? String(evalRaw).trim()
          : "";
    if (!leaderUserId || !evaluateeUserId) {
      continue;
    }
    const ar = o.isAutoGenerated ?? o.is_auto_generated;
...
function mappingRowIsClearlyAuto(m: { isAutoGenerated?: unknown }): boolean {
  const ar = m.isAutoGenerated;
  return (
    ar === true || ar === 1 || ar === "true" || ar === "1" || ar === "True"
  );
}
...
function collectEvaluateesForLeaderCanvas(
  ...
  const connected = buildLeaderConnectedEvaluateeIds(
    board,
    leaderUserId,
    edgesSnapshot,
  );
  ...
  for (const e of board.evaluatees) {
    if (
      normalizeEvaluateePoolSource(e.poolSource) === "extra_pool" &&
      !byId.has(e.userId)
    ) {
      byId.set(e.userId, e);
    }
...
function buildFocusHighlightNodeIds(
  leaderUserId: string,
  edges: Edge[],
): Set<string> {
...
function matchesSearch(name: string, deptNames: string[], q: string): boolean {
...
const LEGEND_LINE_STROKE_CLASS =
  "border-foreground/75 dark:border-foreground/80";
    ...
    >
      ...
      <ul className="mt-2 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-5 sm:gap-y-2">
        <li className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          />
          <span>
            <span className="font-medium text-foreground">점선</span>— 자동
            매핑(조직도 기반). 삭제로 끊을 수 없습니다.
          </span>
        </li>
        <li className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          />
          <span>
            <span className="font-medium text-foreground">실선</span>— 수동
            매핑(직접 연결). 선택 후 삭제할 수 있습니다.
          </span>
        </li>
      </ul>
    </div>
...
function boardToFlow(
  ...
  leaders.forEach((l, i) => {
    const y =
      nL <= 1
        ? Y_TOP + 120
        : Y_TOP + (i / (nL - 1)) * Math.max(320, (nL - 1) * ROW_GAP);
    nodes.push({
      id: `l-${l.userId}`,
      type: "leaderSubject",
  ...
  for (const m of mappings) {
    if (
      !leaderIdSet.has(m.leaderUserId) ||
      !evaluateeIdSet.has(m.evaluateeUserId)
    ) {
      continue;
    }
    const isAuto = mappingRowIsClearlyAuto(m);
    edges.push({
...
function mappingEdgeConnectionKey(e: Pick<Edge, "source" | "target">): string {
...
function manualMappingLinkKey(link: {
  leaderUserId: string;
  evaluateeUserId: string;
}): string {
  return `${link.leaderUserId}\0${link.evaluateeUserId}`;
}

/**
 * 맵은 한 명의 리더만 보이므로 `edges`에는 그 리더의 수동 연결만 있습니다.
 * 저장(전역 PUT) 시에는 서버에 남아 있는 **다른 리더**의 수동 매핑을 빼먹지 않고 합칩니다.
 * (그렇지 않으면 B 화면에서 저장·또는 후보 제거 후 저장 시 A–X 같은 연결이 DB에서 삭제됩니다.)
 */
function buildManualLinksForGlobalSave(
  board: LeaderReviewGlobalMappingBoard | undefined,
  activeLeaderUserId: string | null,
  edges: Edge[],
): Array<{ leaderUserId: string; evaluateeUserId: string }> {
  const fromEdges = edgesToManualLinks(edges);
  if (!board?.mappings || !activeLeaderUserId) {
    return fromEdges;
  }
  const merged = new Map<
    string,
    { leaderUserId: string; evaluateeUserId: string }
  >();
  for (const m of board.mappings) {
    if (!m.leaderUserId?.trim() || !m.evaluateeUserId?.trim()) {
      continue;
    }
    if (mappingRowIsClearlyAuto(m)) {
      continue;
    }
    if (m.leaderUserId === activeLeaderUserId) {
      continue;
    }
    merged.set(manualMappingLinkKey(m), {
      leaderUserId: m.leaderUserId,
      evaluateeUserId: m.evaluateeUserId,
    });
  }
  for (const link of fromEdges) {
    merged.set(manualMappingLinkKey(link), link);
  }
    ...
    if (panTarget.kind === "node") {
      ...
      const groupNodes = getNodes().filter((n) => {
        ...
        return (
          ...
          d.groupIndex === panTarget.groupIndex
        );
      });
...
export default function MultiRaterEvaluateeMapping() {
  ...
  const [focusLeaderUserId, setFocusLeaderUserId] = useState<string | null>(
    null,
  );
  /** 왼쪽에서 리더 선택 시 맵·피평가 목록을 그 리더 연결만으로 한정 */
  const [panelLeaderFilterUserId, setPanelLeaderFilterUserId] = useState<
    string | null
  >(null);
  /** 사이드 목록에서 이름 클릭 시 해당 노드로 뷰 이동 */
  const [panTarget, setPanTarget] = useState<PanTarget>(null);
  /**
   * Locally removed manual edges (source|target). Without this, the board sync effect
   * immediately re-adds the same link from server `mappings` after Delete/Backspace.
   */
  const [suppressedManualKeys, setSuppressedManualKeys] = useState<Set<string>>(
    () => new Set(),
  );
  ...
  const onEdgesChange = useCallback(
    (changes: EdgeChange[]) => {
      setEdges((eds) => {
        ...
        const removedManualKeys: string[] = [];
        for (const ch of filtered) {
          ...
          if ((edge.data as MappingEdgeData | undefined)?.origin === "manual") {
            removedManualKeys.push(mappingEdgeConnectionKey(edge));
          }
        }

        const next = applyEdgeChanges(filtered, eds);
        if (removedManualKeys.length > 0) {
          queueMicrotask(() => {
            setSuppressedManualKeys((prev) => {
              const mergedKeys = new Set(prev);
              for (const k of removedManualKeys) {
                mergedKeys.add(k);
              }
              return mergedKeys;
            });
  ...
  const {
    ...
    isError,
    error,
  } = useQuery<LeaderReviewGlobalMappingBoard, Error>({
    ...
    queryFn: async () => {
      ...
      const raw =
        ...
        unwrapGlobalMappingBoardPayload(
          (boardHttpData as { data?: unknown }).data,
        );
      if (!raw) {
        throw new Error(
          "매핑 보드 응답이 비어 있거나 형식이 올바르지 않습니다.",
        );
      }
      const extraUserIds = new Set(
        collectLeaderReviewExtraPoolUserIdsFromPayload(boardHttpData),
      );
      const normalized = normalizeLeaderReviewMappingBoard({
        ...raw,
        mappings: normalizeMappingsFromApiForBoard(raw.mappings),
      });
  ...
  const panelLeaderFilterResolved = useMemo(() => {
    ...
    return boardRes.leaderSubjects.some(
      (l) => l.userId === panelLeaderFilterUserId,
    )
  ...
  const activeLeaderForSaveRef = useRef(panelLeaderFilterResolved);
  edgesForSaveRef.current = edges;
  activeLeaderForSaveRef.current = panelLeaderFilterResolved;

  useEffect(() => {
    setSuppressedManualKeys(new Set());
  }, [boardRes, panelLeaderFilterResolved]);
  ...
  useEffect(() => {
    ...
    const leaderRecord = boardRes.leaderSubjects.find(
      (x) => x.userId === filterId,
    );
    const leadersPayload = leaderRecord ? [leaderRecord] : [];
    const mappingsPayload = boardRes.mappings.filter(
      (m) => m.leaderUserId === filterId,
    );
    const evaluateesPayload = collectEvaluateesForLeaderCanvas(
      boardRes,
      filterId,
      edges,
    );
    ...
    setEdges((prev) => {
      ...
      for (const e of ed) {
        if (suppressedManualKeys.has(connectionKey(e))) {
          continue;
        }
        merged.set(e.id, e);
      ...
      for (const e of prev) {
        ...
        const isUnsavedManual = e.id.startsWith("conn-");
        const isServerKeyedManual = e.id.startsWith("saved-");
        if (isServerKeyedManual && !suppressedManualKeys.has(ck)) {
          continue;
        }
        if (!isUnsavedManual && !suppressedManualKeys.has(ck)) {
          continue;
        }
    ...
    boardRes,
    panelLeaderFilterResolved,
    filteredLeaderConnectedIdsKey,
    suppressedManualKeys,
    setNodes,
    setEdges,
  ]);
        ...
        (old) => {
          ...
          if (idx >= 0) {
            ...
            const nextRow: EvaluateeUser = {
              ...
              departmentNames:
                depts.length > 0 ? depts : prevRow.departmentNames,
  ...
  const removePoolMutation = useMutation({
    mutationFn: async (variables: {
      ...
      DELETE_leaderReviewEvaluateePoolExtra(
        variables.userId,
        variables.scopeLeaderUserId,
      ),
    onSuccess: (_data, variables) => {
      const { userId, scopeLeaderUserId } = variables;
      toast.success(EXTENDED_POOL_REMOVE_HINT);
      queryClient.setQueryData<LeaderReviewGlobalMappingBoard>(
        ["leader-review-global-evaluatee-mapping-board"],
        (old) => {
          if (!old) {
            return old;
          }
          const scope = scopeLeaderUserId?.trim() ?? "";
          const nextMappings = old.mappings.filter((m) => {
            if (m.evaluateeUserId !== userId) {
              return true;
            }
            if (mappingRowIsClearlyAuto(m)) {
              return true;
            ...
            return m.leaderUserId !== scope;
          });
          const hasAnyMappingForUser = nextMappings.some(
            (m) => m.evaluateeUserId === userId,
          );
          const nextEvaluatees = old.evaluatees.filter((e) => {
            if (e.userId !== userId) {
              return true;
            }
            if (normalizeEvaluateePoolSource(e.poolSource) !== "extra_pool") {
              return true;
            }
            return hasAnyMappingForUser;
          });
          return {
            ...old,
            mappings: nextMappings,
            evaluatees: nextEvaluatees,
            ...(Array.isArray(old.extraPoolUserIds)
              ? {
                  extraPoolUserIds: old.extraPoolUserIds.filter(
                    (id) => id !== userId,
                  ),
                }
              : {}),
  ...
  const onConnect = useCallback(
    (connection: Connection) => {
      ...
      if (
        !connection.source.startsWith("l-") ||
        !connection.target.startsWith("u-")
      ) {
      ...
      const leader = boardRes?.leaderSubjects.find(
        (l) => l.userId === leaderId,
      );
      ...
      const reconnectKey = mappingEdgeConnectionKey({
        source: connection.source,
        target: connection.target,
      });
      setSuppressedManualKeys((prev) => {
        if (!prev.has(reconnectKey)) {
          return prev;
        }
        const next = new Set(prev);
        next.delete(reconnectKey);
        return next;
      });
  ...
  const clearManualEdges = useCallback(() => {
    setEdges((eds) => {
      const manuals = eds.filter(
        (e) => (e.data as MappingEdgeData)?.origin === "manual",
      );
      const keys = manuals.map((e) => mappingEdgeConnectionKey(e));
      if (keys.length > 0) {
        queueMicrotask(() => {
          setSuppressedManualKeys((prev) => new Set([...prev, ...keys]));
        });
      }
      return eds.filter(
        (e) => (e.data as MappingEdgeData)?.origin !== "manual",
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const boardSnapshot =
        queryClient.getQueryData<LeaderReviewGlobalMappingBoard>([
          "leader-review-global-evaluatee-mapping-board",
        ]) ?? boardRes;
      if (!boardSnapshot) {
        throw new Error("매핑 보드가 없습니다.");
      }
      const links = buildManualLinksForGlobalSave(
        boardSnapshot,
        activeLeaderForSaveRef.current,
        edgesForSaveRef.current,
      );
      const body = await PUT_leaderReviewGlobalEvaluateeMappings(links);
  ...
  const syncAutoMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      toast.success(
        res?.message ?? `자동 매핑을 ${n}명의 리더 기준으로 갱신했습니다.`,
      );
      void queryClient.invalidateQueries({
        queryKey: ["leader-review-global-evaluatee-mapping-board"],
      });
  ...
  const evaluateePoolForPanel = useMemo(() => {
    ...
    return collectEvaluateesForLeaderCanvas(
      board,
      panelLeaderFilterResolved,
      edges,
    );
    ...
    <div className="flex flex-col gap-6 p-6">
      <div>
        ...
        <p className="mt-1 max-w-4xl text-sm text-muted-foreground">
          조직도 리더는 왼쪽 목록에서 평가 대상 on/off로 배포 여부를 정할 수
          있습니다. 임원 등 기본 후보에 없는 인원은 오른쪽에서 확장 후보 목록에
          추가한 뒤 연결하세요. 목록의 X는 확장 후보에서 빼고, 왼쪽에서 선택한 리더가
          있으면 그 리더와의 수동 연결(실선)만 함께 지웁니다. 다른 리더의 수동 연결은
          그대로 둡니다. 자동(점선) 연결은 유지됩니다. 점선은 자동·실선은 수동
          매핑입니다. 연결선은
          곡선으로 그려져 겹침을 줄입니다. 피평가자는 조직도상 첫 번째 부서명
          기준으로 묶어 보여 줍니다. 맵은 왼쪽에서 다면평가 대상 리더를 선택한
          뒤에만 표시됩니다.
        </p>
      </div>
                ...
                {panelLeaderFilterResolved ? (
                  <>
                    ...
                    <p className="text-xs text-muted-foreground">
                      오른쪽 부서 헤더의 맵은 그 부서 그룹 전체가 보이도록
                      맞춥니다. 맵에서 리더를 클릭하면 연결만 강조됩니다 (빈 곳
                      클릭 시 해제).
                    </p>
                    <div
                      className={cn(
                  ...
                  >
                    ...
                    <p className="max-w-md text-sm text-muted-foreground">
                      리더를 선택하기 전에는 연결 맵에 노드를 표시하지 않습니다.
                      리더를 고르면 그 리더와 연결된 피평가자와, 후보
                      풀(extra)에만 있어 아직 연결되지 않은 인원이 맵·오른쪽
                      목록에 나타납니다.
                    </p>
                  </div>
                )}
                      ...
                      <Collapsible key={g.key} defaultOpen={g.groupIndex < 4}>
                        <div className="overflow-hidden rounded-md border border-border/70 bg-muted/15">
                          <div className="flex items-stretch gap-0 border-b border-border/50 bg-muted/30">
                            ...
                            >
                              <MapPin className="mr-1 size-3.5" aria-hidden />맵
                            </Button>
                          </div>
                          <CollapsibleContent>
                                ...
                                >
                                  ...
                                  {e.poolSource === "extra_pool" ? (
                                    <Button
                                      ...
                                      onClick={() =>
                                        removePoolMutation.mutate({
                                          userId: e.userId,
                                          scopeLeaderUserId:
                                            panelLeaderFilterResolved,
                                        })
                                      }
                                      title={EXTENDED_POOL_REMOVE_HINT}
                                      aria-label={`${e.koreanName}: ${EXTENDED_POOL_REMOVE_HINT}`}
                                    >
                                      <X className="size-4" />
                                    </Button>
                                  ) : null}
      ...
      <Dialog open={addPoolOpen} onOpenChange={setAddPoolOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {
                "\uc784\uc6d0 \ub4f1 \uae30\ubcf8 \ud6c4\ubcf4\uc5d0 \uc5c6\ub294 \uc0ac\uc6a9\uc790\ub97c \uac80\uc0c9\ud574 \ud655\uc7a5 \ud6c4\ubcf4 \ubaa9\ub85d\uc5d0 \ucd94\uac00\ud569\ub2c8\ub2e4. \ud655\uc7a5 \ubaa9\ub85d\uc5d0 \uc5c6\uc5b4\ub3c4 \uc804\uc5ed \ub9e4\ud551\uc5d0 \uc774\ubbf8 \ud3ec\ud568\ub41c \ud53c\ud3c9\uac00\uc790\ub294 \uc218\ub3d9 \uc5f0\uacb0\uc744 \uc720\uc9c0\xb7\uc800\uc7a5\ud560 \uc218 \uc788\uc2b5\ub2c8\ub2e4."
              }
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
